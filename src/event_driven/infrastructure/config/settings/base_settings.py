...
def track_origin(data: dict | list, origin: str, prefix: str = "", registry: dict | None = None) -> dict[str, str]:
...
class BaseTraceableSettings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        current_env = os.getenv(env_prefix, "base")
        ...
        return (
            ...
            track_source(init_settings, "init"),
            *active_sources,
    ...
    def get_origins(self) -> dict:
        """Returns the origins from each variable of the model."""
        field_names = self.model_dump().keys()
        return {k: self.get_origin(k) for k in field_names}
