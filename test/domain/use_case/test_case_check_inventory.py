...
class TestCheckInventoryUseCase(unittest.TestCase):
    ...
    def test_execute_successful_order_creation(self) -> None:
        ...
        payload = CartItemsModel(id="event-uuid-123", items=[{"product_id": "prod-1", "quantity": 2}])

        # Act: Execute the use case
        result: OrderCreatedModel | None = self.use_case.execute(payload)
    ...
    def test_execute_raises_value_error_when_items_are_empty(self) -> None:
        ...
        payload = CartItemsModel(id="event-uuid-123", items=[])
    ...
    def test_execute_handles_exception_gracefully(self) -> None:
        ...
        payload = CartItemsModel(id="event-uuid-123", items=[{"product_id": "prod-1", "quantity": 1}])
